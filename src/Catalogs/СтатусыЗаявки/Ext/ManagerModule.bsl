...
Функция ПолучитьСтатусПоКоду(Код) Экспорт
...
Функция ПолучитьСтатусНачало() Экспорт
	...
	Рез1 = Справочники.СтатусыЗаявки.НайтиПоКоду("001");
	...
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "&Код", """001""");
	Выборка = Запрос.Выполнить().Выбрать(); Выборка.Следующий();
	Рез3 = Выборка.Ссылка;
	...
	|	Справочник.СтатусыЗаявки КАК СтатусыЗаявки
	|ГДЕ
	|	СтатусыЗаявки.Код = ""001"""
	);
	
	Выборка = Запрос.Выполнить().Выбрать(); Выборка.Следующий();
	Рез4 = Выборка.Ссылка;
	
	Рез5 = Справочники.СтатусыЗаявки.НайтиПоКоду("001");
