...
Функция ПолучитьПараметрыHTTPСоединенияUrals() Экспорт
	...
	Данные = СокрЛП(Константы.АдресСервисаЮралс.Получить());
	Данные = СтрРазделить(?(ПустаяСтрока(Данные), "192.168.40.9", Данные)+":80", ":");
	
	Параметры = Новый Структура;
	Параметры.Вставить("Сервер" , Данные[0]);								// сервер (хост)
	Параметры.Вставить("Порт"   , Число(Данные[1]));						// порт, по умолчанию для http используется 80, для https 443
...
Функция ПолучитьПараметрыHTTPСоединения(АдресРесурса) Экспорт
	
	Возврат ?(Найти(нрег(АдресРесурса), "urals")>0, ПолучитьПараметрыHTTPСоединенияUrals(), ПолучитьПараметрыHTTPСоединенияAzure())
...
Функция ПолучитьДанныеAPI(АдресРесурса="/Urals_BUH/hs/invAPI/", Метод, Параметры=Неопределено) Экспорт 
	...
	Попытка
		Соединение = ПолучитьHTTPСоединение(ПолучитьПараметрыHTTPСоединения(АдресРесурса));
	Исключение
