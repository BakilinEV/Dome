...
&НаСервере
Процедура ОбновитьОстатокНоменклатуры()
...
&НаСервереБезКонтекста
Функция ПолучитьСписокВыбораКаталожныхНомеровСервер(Номенклатура)
...
&НаКлиенте
Процедура ОбновитьСписокВыбораКаталожныхНомеров()
	
	ТекущиеДанные = Элементы.Товары.ТекущиеДанные;
	
	СписокВыбора = Элементы.ТоварыНоменклатураАртикул.СписокВыбора;
	СписокВыбора.ЗагрузитьЗначения(ПолучитьСписокВыбораКаталожныхНомеровСервер(ТекущиеДанные.Номенклатура));
	
	Если ПустаяСтрока(ТекущиеДанные.Артикул) И СписокВыбора.Количество()>0 Тогда
