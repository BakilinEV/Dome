...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Для Каждого СтрокаЗаказа Из Товары Цикл
		...
		Если Не СтрокаЗаказа.Номенклатура.Пустая() Тогда
			...
			Представление  = СокрЛП(СтрокаЗаказа.Номенклатура)+" ";
			СтрокаОписания.Наименование = Лев(Представление, Найти(Представление, " ")-1);
...
Процедура ПриЗаписи(Отказ)
	...
	|			И (ЗаявкаСтатусы.Статус = &СтатусНачало)
	|ГДЕ
	...
	Запрос.УстановитьПараметр("СтатусНачало", Справочники.СтатусыЗаявки.СтатусНачало());
	...
	Если Не Результат.Пустой() Тогда
		...
		Пока Выборка.Следующий() Цикл
			...
			Сообщить("Записан "+МенеджерЗаписи.ИдентификаторСтроки);
		КонецЦикла;
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	Для Каждого СтрокаТаблицы Из ТаблицаДанных Цикл
		...
		Если РегистрыСведений.ЗаявкаСтатусы.ПолучитьСтатус(СтрокаТаблицы.ИдентификаторСтроки) = Справочники.СтатусыЗаявки.СтатусНачало() Тогда
