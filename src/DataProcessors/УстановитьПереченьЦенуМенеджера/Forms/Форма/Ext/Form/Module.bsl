...
Процедура ЗаполнитьЦеныНаСервере(СтрокаДанных, Список=Неопределено)
	...
	Иначе
		...
		ИначеЕсли СтрокаДанных.Перечень = 3 Тогда
			...
			Запрос.УстановитьПараметр("ИдентификаторСтроки", ТаблицаID.Выгрузить(Новый Структура("Номенклатура", СтрокаДанных.Номенклатура), "ИдентификаторСтроки").ВыгрузитьКолонку("ИдентификаторСтроки"));
...
Процедура ЗаписатьДанныеНаСервере()
	...
	ТаблицаПоиска = ТаблицаДанных.Выгрузить();
	
	Для Каждого СтрокаID Из ТаблицаID Цикл
		
		СтрокаДанных = ТаблицаПоиска.Найти(СтрокаID.Номенклатура, "Номенклатура");
		...
		Если СтрокаДанных.Количество = 0 Тогда
			...
			РегистрыСведений.ЗаявкаСтатусы.УстановитьСтатус(СтрокаID.ИдентификаторСтроки, Справочники.СтатусыЗаявки.СтатусОтмена());
			
		ИначеЕсли СтрокаДанных.Перечень <> 0
			...
			Запись.ИдентификаторСтроки = СтрокаID.ИдентификаторСтроки;
...
&НаСервере
Процедура ТаблицаДанныхЦенаНачалоВыбораНаСервере(СтрокаДанных, Список)
...
Процедура ТаблицаДанныхЦенаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ТаблицаДанныхЦенаНачалоВыбораНаСервере(Элементы.ТаблицаДанных.ТекущиеДанные, ДанныеВыбора);
...
Процедура ОбновитьДанныеНаСервере()
	...
	|		ПО ЗаявкаСтроки.ИдентификаторСтроки = ЗаявкаЦеныСрезПоследних.ИдентификаторСтроки
	|ГДЕ
	|	ЗаявкаСтатусыСрезПоследних.Статус.Код <> ""000""
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	ТаблицаДанных.Загрузить(Результат[0].Выгрузить());
	ТаблицаID.Загрузить(Результат[1].Выгрузить());
